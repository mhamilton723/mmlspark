...
class HTTPSuite extends TestBase with FileReaderUtils {
  ...
  test("stream from HTTP", TestBase.Extended) {
    ...
    def sendRequest(map: Map[String, Any]): HttpPost = {
      val post = new HttpPost("http://localhost:8889/foo")
    ...
    posts.zip(correctResponses).foreach(p => assert(receiveRequest(p._1) === "{\"newCol\":"+ p._2 + "}"))
    q1.stop()
  }
